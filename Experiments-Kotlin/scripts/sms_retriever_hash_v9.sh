...
USAGE="Usage: sms_retriever_hash.sh --package package_name --keystore keystore_file --keystore_password password"
...
if [[ "$1" != "--keystore_password" ]]; then
  echo "Error: expected --keystore_password as third parameter"
  exit 1
fi
keystore_password="$2"
shift 2
...
echo "keystore file: keystore_password"
echo 
...
cert=$(keytool -list -rfc -keystore $keystore -storepass $keystore_password | sed  -e '1,/BEGIN/d' | sed -e '/END/,$d' | tr -d ' \n' | base64 --decode | xxd -p | tr -d ' \n')
